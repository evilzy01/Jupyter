...
public class TicketMasterAPI {
	...
	private static final String API_KEY = "BVVRzrZs3anQFBxkNMLRtahspRKJllh2";
	
	/**
	 * Search Function to get Events by lat & lon
	 */	
...
/**
 * Clean the search information, only select information we want!
 * @param event
	...
	private String getImageUrl(JSONObject event) throws JSONException {
		if (!event.isNull("images")) {
			...
			for (int i = 0; i < images.length(); ++i) {
				...
				if (!image.isNull("url")) {
					return image.getString("url"); // Return first URL ???
				}
			}
		}
